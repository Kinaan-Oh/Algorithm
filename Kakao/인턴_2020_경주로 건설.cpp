// 출제의도: "bfs", Time Complexity: O(), Space Complexity: O().
// 이 문제의 경우 board상 임의의 한 지점에 대한 탐색여부에 대한 결정은 단순히 과거에 방문(visited)사실만으로 판단이 불가. 과거에 방문이 되었더라도, 1. 방향(dir) 2. 비용 을 고려하여 탐색여부를 결정. 
// 여기서, 2. 비용 은 "일반적인 bfs에서 과거에 방문된 지점에 대한 비용은 나중에 방문할 경우 반드시 더 비싸다" 라는 가정이 성립되지 않을 수 있음. 코너링의 비용이 500으로 이동비용(100) 보다 훨씬 비싸므로,
// hop(홉)이 더 많다고 해서 비용이 반드시 비싸다고 할 수는 없게됨. 따라서, 해당 지점에 대해 방문 비용을 저장해놓고, 나중에 더 저렴한 비용으로 방문(충분히 가능하다)하게 되면 업데이트하고 탐색을 진행한다.
