// 출제의도: "우선순위 큐, SPN"
// 나의 풀이는 정해는 아니지만(타이머 인터럽트 기반 스케줄러를 변수 clk를 통해 구현한 것이라 비효율일 수밖에 없음), OS강의 수강시 구현했던 프로세스 스케줄러의 개념을 복습겸 구현.
// OS강의에서 구현했던 스케줄러는 멀티레벨 피드백 큐 방식이라, 여기서 구현한 SPN(Shortest Process Next)의 구현은 비교적 많이 쉬운편.
